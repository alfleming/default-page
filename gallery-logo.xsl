              	...
              	<xsl:variable name="numbered" select="position()"/>
                    <div class="col-sm-3 thumb-collector">
                        <div class="thumbnail thumbnail-gallery">
                            <a href="#"><xsl:if test="URL[not(.='')]"><xsl:attribute name="href"><xsl:value-of select="URL" disable-output-escaping="yes"/></xsl:attribute></xsl:if>
                                <div class="gallery-thumbnails" style="background:url({$ROOTDIR}/getimage.asp?id={/ROOT/OBJECT/ID}&amp;filename={NAME}&amp;mode=6) no-repeat center center; background-size:contain; margin-bottom:0; height:200px;"></div>
                            </a>
                        </div>
                    </div>
                </xsl:for-each>
